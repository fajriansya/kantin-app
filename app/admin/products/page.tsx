          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600">Produk</th>
                ...
                {products.length === 0 ? (
                  ...
                  products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded-full text-xs">{product.category || "-"}</span>
                      </td>
                      <td className="px-6 py-4 font-semibold text-gray-900">{formatPrice(product.price)}</td>
                      <td className="px-6 py-4">
                        <span className={`text-sm ${product.stock && product.stock > 0 ? 'text-green-600' : product.stock === 0 ? 'text-red-600' : 'text-gray-500'}`}>
                          {product.stock && product.stock > 0 ? `${product.stock} tersisa` : product.stock === 0 ? "Habis" : "Tak terbatas"}
                        </span>
                      </td>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            ...
            <form onSubmit={handleUpdateProduct} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="number" value={editingProduct.stock === null ? "" : editingProduct.stock} onChange={(e) => setEditingProduct({...editingProduct, stock: e.target.value === "" ? null : parseInt(e.target.value)})} placeholder="Kosongkan jika tak terbatas" className="w-full px-3 py-2 border rounded-lg" />
